...
public class TagsProcessor extends ProcessorAbstract {
    private static final Logger logger = LogManager.getLogger(MethodHandles.lookup().lookupClass());

    @Override
    public void process(FsCrawlerContext ctx) throws ProcessingException {
        
        var mergedDoc = this.getMergedJsonDoc(ctx.getDoc(), ctx.getTags());
        ctx.setDoc(mergedDoc);

    }

    private Doc getMergedJsonDoc(Doc doc, InputStream tags) throws ProcessingException {
        var span = tracer.spanBuilder("tags").startSpan();
        try (Scope scope = span.makeCurrent()) {
            ...
            throw new ProcessingException("Error parsing tags", e);
        } finally {
            span.end();
        }
    }
