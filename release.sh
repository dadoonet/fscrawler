...
if [ $? -eq 0 ]; then
	if promptyn "Tag fscrawler-$RELEASE_VERSION already exists. Do you want to remove it?"; then
...
if promptyn "Is message ok?"; then
...
RELEASE=0
if promptyn "Everything is ready and checked. Do you want to release now?"; then
	RELEASE=1
...
if [ ${DRY_RUN} -eq 0 ]; then
	...
	if promptyn "Is the DockerHub repository ok?"; then
...
if [ ${RELEASE} -eq 1 ]; then
	...
	if [ ${DRY_RUN} -eq 0 ]; then
		...
		if promptyn "Do you want to announce the release?"; then
	...
	if promptyn "Do you want to remove $RELEASE_BRANCH branch and fscrawler-${RELEASE_VERSION} tag?"; then
		git branch -q -D ${RELEASE_BRANCH}
		git tag -d fscrawler-${RELEASE_VERSION}
	fi
fi
