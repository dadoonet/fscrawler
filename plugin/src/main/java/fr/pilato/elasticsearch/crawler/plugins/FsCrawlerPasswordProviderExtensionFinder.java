...
/**
 * Extension finder that only reads the FsCrawler password provider SPI file ({@link #SERVICE_RESOURCE}), instead of
 * scanning all META-INF/services.
 */
public class FsCrawlerPasswordProviderExtensionFinder extends AbstractExtensionFinder {

    private static final Logger log = LogManager.getLogger(FsCrawlerPasswordProviderExtensionFinder.class);

    /** Only this SPI file is read (FsCrawler password provider implementations). */
    public static final String SERVICE_RESOURCE =
            "META-INF/services/" + FsCrawlerExtensionPasswordProvider.class.getName();

    private static final Pattern COMMENT = Pattern.compile("#.*");
    private static final Pattern WHITESPACE = Pattern.compile("\\s+");

    public FsCrawlerPasswordProviderExtensionFinder(PluginManager pluginManager) {
        super(pluginManager);
    }

    @Override
    public Map<String, Set<String>> readClasspathStorages() {
        log.debug("Reading PasswordProvider extensions from classpath ({})", SERVICE_RESOURCE);
    ...
    public Map<String, Set<String>> readPluginsStorages() {
        log.debug("Reading PasswordProvider extensions from plugins ({})", SERVICE_RESOURCE);
        Map<String, Set<String>> result = new LinkedHashMap<>();
