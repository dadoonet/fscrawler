...
public class TikaInstance {
     ...
     */
    public TikaInstance(Fs fs) {
        boolean ocrActivated = fs.getOcr().isEnabled();
    ...
    String extractText(int indexedChars, InputStream stream, Metadata metadata, String password) throws IOException,
            TikaException {
        ...
        try (stream) {
            ...
            // If the password was provided, it means it was wrong
            if (password != null) {
                logger.debug("The document {} is encrypted and the provided password seems to be wrong: {}", resourceName, e.getMessage());
            } else {
                logger.debug("The document {} is encrypted and no password was provided: {}", resourceName, e.getMessage());
            }
    ...
    public LanguageDetector langDetector() {
        return detector;
    }

    /**
     * Plain text password provider
     */
    private static class StandardPasswordProvider implements PasswordProvider {
        private final String password;

        StandardPasswordProvider(String password) {
