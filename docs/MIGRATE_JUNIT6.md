...
| `SystemThreadFilter` | From randomizedtesting-jupiter: ignores system thread group, ForkJoinPool.commonPool threads, and known safe daemon threads (JFR timer, AWT-AppKit, process reaper, etc.) |
| `ForkJoinPoolThreadFilter` | Custom `Predicate<Thread>`: ignores all `ForkJoinPool-*` worker threads (e.g., ForkJoinPool-1-worker-*). The built-in `SystemThreadFilter` only catches ForkJoinPool.commonPool-*, so we need this for numbered pools. |
