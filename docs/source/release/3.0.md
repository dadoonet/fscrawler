...
- FSCrawler now ships with Apache Tika's `tika-vlm` module: OCR can be delegated to a Vision Language
  ...
  Gemini, configured via a custom Tika configuration file. See {ref}`vlm-ocr`. Thanks to dadoonet.
- Add option to set path to custom tika config file. See {ref}`local-fs-settings`. Thanks to iadcode.
>>>>>>> 8e52e4f9 (feat(tika): ✨ add tika-vlm (OpenAIVLMParser) support and documentation)
