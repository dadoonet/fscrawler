...
- Add option to set path to custom tika config file. See {ref}`local-fs-settings`. Thanks to iadcode for the original
  ...
  **Note**: since the Apache Tika 4 upgrade, the configuration file must be a Tika JSON configuration —
  ...
  converted. See {ref}`local-fs-settings`.
...
- The top-level `server.*` settings (`hostname`, `port`, `username`, `password`, `pem_path`) are
  deprecated. Use `fs.ssh.*` or `fs.ftp.*` instead (same field names as the REST `_document` payload).
  FSCrawler logs a WARN with the replacement key when a deprecated field is used. Thanks to dadoonet.
