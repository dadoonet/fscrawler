...
SSH/FTP connection details belong under `fs.ssh` / `fs.ftp` (see {ref}`ssh-settings` and
{ref}`ftp-settings`). The top-level `server.*` block is deprecated and will be removed
in a future version.
