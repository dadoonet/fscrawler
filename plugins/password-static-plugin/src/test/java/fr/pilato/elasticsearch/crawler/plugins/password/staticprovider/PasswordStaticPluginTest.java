...
class PasswordStaticPluginTest extends AbstractFSCrawlerTestCase {

    @Test
    void providerYieldsConfiguredPasswordsInOrder() {
        ...
        Assertions.assertThatNoException().isThrownBy(() -> {
            ...
            provider.start(settings, type -> null);

            Assertions.assertThat(provider.getType()).isEqualTo("static");

            try (var session = provider.open(documentPath)) {
                Assertions.assertThat(session.next()).contains("a");
                Assertions.assertThat(session.next()).contains("b");
    ...
    private static FsCrawlerExtensionPasswordProvider instantiateProvider() throws Exception {
        return (FsCrawlerExtensionPasswordProvider) Class.forName(
                        "fr.pilato.elasticsearch.crawler.plugins.password.staticprovider.PasswordStaticPlugin$Provider")
